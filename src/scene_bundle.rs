...
impl<'a> Scene<'a> {
    pub fn new_from_bundle(path:&str) -> Scene<'a> {
        let path = path::Path::new(path).join("scene.xml");
...
fn process_scene_attributes<'a>(_scene:&Scene, attributes:Vec<xml::attribute::OwnedAttribute>) -> node::Node<'a> {
